...
type BaseAI(board: Board) = 
    abstract member CreateAI: Board -> AI
    ...
    interface AI with
        ...
        member x.Moves 
            with get() =
                ...
                let possibleBoards = possibleMoves |> Seq.map (fun c -> (c, getFitness c.Pos board.Player, getFitness c.Pos opponent))  |> Seq.toList
...
type EasyAI(board) =
    ...
    override x.CreateAI board = EasyAI(board) :> AI

    override x.RowScores 
        with get() = 
            [|  Int32.MaxValue |> float; // length 5
                512.0;                   // length 4, rank 2
...
type MediumAI(board) =
    ...
    override x.CreateAI board = MediumAI(board) :> AI
